            ...
            while (idx < len)
            {
                if (data[idx] == '[')
                {
                    if (tempMsg.Length > 0)
                    {
                        ...
                        msg.AddData("text", NapCatCharEncoder.Decode(tempMsg.ToString()));
                        ret.AddMessage(msg);
                    }
                    ...
                    if (cqData.Length > 0)
                    {
                        if (System.Enum.TryParse<CqCode>(cqData[0].Replace("CQ:", ""), out var type))
                        {
                            ...
                            for (int i = 1; i < cqData.Length; i++)
                            {
                                ...
                                if (equalIndex != -1) 
                                    msg.AddData(cqData[i].Substring(0, equalIndex), NapCatCharEncoder.Decode(cqData[i].Substring(equalIndex + 1, cqData[i].Length - equalIndex - 1)));
                            }
                            ret.AddMessage(msg);
                        }
                    }
